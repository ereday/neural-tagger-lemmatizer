...
#    cd1 = cd1 .+ output_encodings_c
    for i=1:decoder_ts
        ...
        ydx,output_encodings_h,output_encodings_c,_ = rnnforw(rsettings[end],weights[end],x2,output_encodings_h,output_encodings_c)
        x3 = dropout(ydx,pdrop;training=true)
        yd2,hd1,cd1,_ = rnnforw(rsettings[3],weights[5],x3,hd1,cd1)
...
function predict(weights,rsettings,data,rov,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    ...
    for i=1:wordnumber
        ...
        for j=1:decoder_ts
            ...
            yx,ho,co,_ = rnnforw(rsettings[end],weights[end],y1,ho,co)
            y2,hd1_single,cd1_single,_ = rnnforw(rsettings[3],weights[5],yx,hd1_single,cd1_single)
