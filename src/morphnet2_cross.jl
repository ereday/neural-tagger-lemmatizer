...
function parseargs()
    ...
    @add_arg_table s begin
        ("--datafiles_low";arg_type=String ;nargs='+';help="parsed data if available")
        ("--datafiles_high";arg_type=String ;nargs='+';help="parsed data if available")
...
function main()
    ...
    opts[:model] = "morphnet2_cross.jl"
    Logging.configure(level=DEBUG)
    Logging.configure(output=open(opts[:logfile], "a"))
    enc_data_low,enc_data_high,c2i,o2i = prepare_data(opts)
    ...
    if opts[:mode] == 2
        ...
        besties = zeros(2)
        data_low  = minibatch(enc_data_low,opts,o2i;st=false)
        words1 = map(x->map(y->y[1],x),enc_data_low[3])
        words  = map(x->map(y->join(map(z->i2c[z],y),""),x),words1)
        generate(weights,rsettings,data_low[3],words,i2o,i2oarr,i2c,opts)
        return 1
    elseif isfile(opts[:bestmodel])
        ...
        info("input vocabulary length:",length(c2i))
        info("output vocabulary length:",length(o2i))
        data_low = minibatch(enc_data_low,opts,o2i)
        data_high = minibatch(enc_data_high,opts,o2i)
        besties = zeros(2)
    ...
    data_low_x = Array{Any}(3)
    data_low_x[1] = shuffle(data_low[1])
    data_low_x[2] = data_low[2]
    data_low_x[3] = data_low[3]
    dlss,dacc = evaluate(weights,rsettings,data_low_x[2],i2o,i2oarr,opts)
    info(@sprintf "[dev-0] acc:%.4f lss:%.4f"  (100*dacc[1])/dacc[2] dlss[1]/dlss[2])
    tlss,tacc = evaluate(weights,rsettings,data_low_x[3],i2o,i2oarr,opts)
    info(@sprintf "[tst-0] acc:%.4f lss:%.4f"  (100*tacc[1])/tacc[2] tlss[1]/tlss[2])

    patiance = [opts[:patiance]]
    for i=1:opts[:epoch]
        lss = train(weights,rsettings,params,(data_low_x[1],data_high[1]),opts)
        info(@sprintf "epoch:%d trnlss:%.4f:" i lss[1][1]/lss[1][2])
        # high resource evaluation
        dlss,dacc = evaluate(weights,rsettings,data_high[2],i2o,i2oarr,opts)
        info(@sprintf "[dev-%d] acc:%.4f lss:%.4f" i (100*dacc[1])/dacc[2] dlss[1]/dlss[2])
        tlss,tacc = evaluate(weights,rsettings,data_high[3],i2o,i2oarr,opts)
        ...
        tlss,tacc = evaluate(weights,rsettings,data_low_x[3],i2o,i2oarr,opts)
        info(@sprintf "[tst-%d] acc:%.4f lss:%.4f" i (100*tacc[1])/tacc[2] tlss[1]/tlss[2])
        # modify optimizer based on low resource dev-set performance
...
function train(weights,rsettings,params,data_all,opts;N=1000)
    low_resource  = data_all[1][1:N]
    high_resource = data_all[2]

    lss_high = _train(weights,rsettings,params,high_resource,opts)
    lss_low  = _train(weights,rsettings,params,low_resource,opts)
    return lss_low,lss_high
end

function train2(weights,rsettings,params,data_all,opts;N=1000)

    low_resource = data_all[1][1:N]
    high_resource = data_all[2]

    data = Any[]
    ixs = randperm(length(low_resource)+length(high_resource))
    for ix in ixs
        if ix > length(low_resource)
            push!(data,high_resource[ix-length(low_resource)])
        else
            push!(data,low_resource[ix])
        end
    end

    lss = _train(weights,rsettings,params,data,opts)
    return lss,lss
end

function _train(weights,rsettings,params,data,opts)
...
function loss(weights,rsettings,data,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    ...
    for i=1:decoder_ts
        ...
        #yd2 = dropout(yd2,pdrop) ### Diger kodda burada hd1'e uygulamisim dropout'u bir bak buna tekrar
