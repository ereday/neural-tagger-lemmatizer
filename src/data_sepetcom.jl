...
function create_vocabulary(cdf)
    ...
    get!(c2i,UNKTAG,length(c2i)+1)
    get!(c2i,MASK,length(c2i)+1)
    get!(c2i,BOW,length(c2i)+1)
    get!(c2i,EOW,length(c2i)+1)
    ...
    for cf in cdf
        ...
        for sentence in cf
            ...
            for (w,l,t) in sentence
                ...
                elemma = map(x->get(c2i,x,c2i[UNKTAG]),collect(l))
                etag   = map(x->get(o2i,x,o2i[UNKTAG]),t)
