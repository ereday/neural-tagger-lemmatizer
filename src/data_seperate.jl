...
function create_vocabulary(cdf)
    ...
    for (ix,cf) in enumerate(cdf)
        for sentence in cf
            for (w,l,t) in sentence
                map(x->get!(t2i,x,length(t2i)+1),t)
            end
        end
    end
    ...
    # tag dictionary
    get!(t2i,UNKTAG,length(t2i)+1)
    get!(t2i,MASK,length(t2i)+1)
    get!(t2i,BOW,length(t2i)+1)
    get!(t2i,EOW,length(t2i)+1)

    return c2i,o2i,t2i
end

function encode_data(cdf,c2i,o2i,t2i)
    edf = Any[]
    for cf in cdf
        esentences = Any[]
        for sentence in cf
...
function prepare_data(opts)
    ...
    c2i,o2i,t2i = create_vocabulary(cdf)
    edf = encode_data(cdf,c2i,o2i,t2i)
    return edf,c2i,o2i,t2i
end
