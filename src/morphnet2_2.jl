...
function main()
    ...
    opts[:model] = "morphnet2_sondeneme.jl"
...
function loss(weights,rsettings,data,ho,co,o2i;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
...
function predict(weights,rsettings,data,rov,ho,co,o2i;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    input,bs,indices,labels,outmasks,tagrange,decoder_start = data
    total = 0.0; count = 0.0
    omask = convert(KnetArray{Float32},outmasks)
    # Word Encoder
