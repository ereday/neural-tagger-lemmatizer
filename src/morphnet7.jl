...
#    cd1 = cd1 .+ output_encodings_c
    for i=1:decoder_ts
        ...
        yd1,hd1,cd1,_ = rnnforw(rsettings[3],weights[5],rnn2input,hd1,cd1)
        x2 = dropout(yd1,pdrop;training=true)

        yd2,decoder_h1,decoder_c1,_ = rnnforw(rsettings[2],weights[4],x2,decoder_h1,decoder_c1)
        x3 = dropout(yd2,pdrop;training=true)

        ydx,output_encodings_h,output_encodings_c,_ = rnnforw(
            rsettings[end],weights[end],x3,output_encodings_h,output_encodings_c)
        output_encodings_h = dropout(output_encodings_h,pdrop;training=true)
        output_encodings_h = reshape(output_encodings_h,
                                     size(output_encodings_h,1),size(output_encodings_h,2))
        ypred = weights[10] * output_encodings_h .+ weights[11] #b
        # loss calculation
        ynorm = logp(ypred,1)
...
function predict(weights,rsettings,data,rov,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    ...
    for i=1:wordnumber
        ...
        for j=1:decoder_ts
            ...
            y1,hd1_single,cd1_single,_ = rnnforw(rsettings[3],weights[5],rnn2input,
                                                 hd1_single,cd1_single)

            y2,dec_h1_s,dec_c1_s,_ = rnnforw(rsettings[2],weights[4],y1,dec_h1_s,dec_c1_s)
            yx,ho,co,_ = rnnforw(rsettings[end],weights[end],y2,ho,co)

            ypred = weights[10] * yx .+ weights[11]
            ynorm = logp(ypred,1)
            index = labels[i,j]
...
function initweights(o,c2i,o2i,init)
    ...
    r23,wdec3 = rnninit(o[:hs][3],o[:hs][3];rnnType=:lstm,seed=o[:seed]) #12
    push!(rsettings,r23)
    push!(w,wdec3)
