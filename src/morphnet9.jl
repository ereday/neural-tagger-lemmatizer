...
function generate(weights,rsettings,data,words,i2o,i2oarr,i2c,o2i,opts;conlluformat=false)
    ...
    for i=1:T
        ho[:] = 0.0;co[:] = 0.0
        preds = predict(weights,rsettings,data[i],i2o,ho,co,o2i;lss=lssval)
        if conlluformat
...
function main()
    ...
    opts[:model] = "morphnet_alloutputs2.jl"
    ...
    if opts[:mode] == 2
        ...
        generate(weights,rsettings,data[3],words,i2o,i2oarr,i2c,o2i,opts)
        return 1
    end
    besties = zeros(2)
    dlss,dacc = evaluate(weights,rsettings,data[2],i2o,i2oarr,o2i,opts)
    info(@sprintf "[dev-0] acc:%.4f lss:%.4f"  (100*dacc[1])/dacc[2] dlss[1]/dlss[2])
    patiance = [opts[:patiance]]
    for i=1:opts[:epoch]
        lss = train(weights,rsettings,params,data[1],o2i,opts)
        info(@sprintf "epoch:%d trnlss:%.4f:" i lss[1]/lss[2])

        dlss,dacc = evaluate(weights,rsettings,data[2],i2o,i2oarr,o2i,opts)
        info(@sprintf "[dev-%d] acc:%.4f lss:%.4f" i (100*dacc[1])/dacc[2] dlss[1]/dlss[2])
        tlss,tacc = evaluate(weights,rsettings,data[3],i2o,i2oarr,o2i,opts)
...
function train(weights,rsettings,params,data,o2i,opts)
    lssval = zeros(Float32,2)
    T = length(data)
    datax = shuffle(data)
    ho = KnetArray(zeros(Float32,opts[:hs][4],1,1))
    ...
    for i=1:T
        ...
        grads = lossgradient(weights,rsettings,datax[i],ho,co,o2i;pdrop=opts[:pdrop],lss=lssval)
        update!(weights,grads,params)
    end
    return lssval
end

function evaluate(weights,rsettings,data,i2o,i2oarr,o2i,opts)
    lssval  = zeros(Float32,2)
...
function loss(weights,rsettings,data,ho,co,o2i;pdrop=0.0,lss=nothing,preds=nothing)
    ...
    for i=1:wordnumber
        all_ho[i]  = ho[:,:,end]
        all_co[i]  = co[:,:,end]
        tags_part  = labels[i,tagrange[i]]
        push!(tags_part,o2i[EOW])
        outemb0    = weights[2][:,tags_part]
...
function predict(weights,rsettings,data,rov,ho,co,o2i;pdrop=0.0,lss=nothing,preds=nothing)
    input,bs,indices,labels,outmasks,tagrange,decoder_start = data
    total = 0.0; count = 0.0
    omask = convert(KnetArray{Float32},outmasks)
    # Word Encoder
