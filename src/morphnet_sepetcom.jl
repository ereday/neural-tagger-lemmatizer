# seperate lemma and posmorp prediction but
# produce posmorp composit way. (in single step)
using Knet,ArgParse,Logging,JLD
include("data_sepetcom.jl")
...
function main()
    ...
    # seperate tag and lemma
    opts[:model] = "morphnet_sepetcom.jl"
    ...
    if opts[:mode] == 2
        ...
        besties = zeros(2)
        data  = minibatch(enc_data,opts,c2i,o2i;st=false)
    ...
    elseif isfile(opts[:bestmodel])
        ...
        info("input vocabulary length:",length(c2i))
        info("output vocabulary length:",length(o2i))
        data = minibatch(enc_data,opts,c2i,o2i)
        besties = zeros(2)
...
function loss(weights,rsettings,data,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    input,bs,indices,lemma_labels,outmasks,posmorph_labels,decoder_start = data
    total = 0.0; count = 0.0
...
function initweights(o,c2i,o2i,init)
    ...
    # output layer (pos)
    push!(w,init(length(o2i),(2o[:hs][2]+o[:hs][4]))) #10
...
function minibatch(edf,opts,c2i,o2i;st=true)
    all_batches = Any[]
    for df in edf
        batches = _minibatch(df,opts,st,c2i)
        push!(all_batches,batches)
    end
    return all_batches
end

# buraya gelen o2i aslinda c2i ile ayni. c2i benim icin lemmanin o2isi ayni zamanda
function _minibatch(df,opts,st,o2i)
    ...
    for i=1:length(sorted_df)
        ...
        # output-posmorph
        posmorph_labels  = map(x->x[3],sorted_df[i])
        # no need to put eow because single step
        decoder_start = [o2i[BOW] for k=1:length(lemma_labels)]
        maxoutlen     = maximum(map(x->length(x),lemma_labels))
        outmasks      = ones(Float32,maxoutlen,length(lemma_labels))
        ...
        posmorph_labels_2d = transpose(hcat(posmorph_labels...))
        push!(batches,(batch_words,batchsizes,reverse_indices,lemma_labels_2d,outmasks_2d,posmorph_labels_2d,decoder_start))
    end
    return batches
end
