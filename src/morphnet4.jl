# composite output encoder ? 
# use diff embeddings, maybe pretraining ? 
...
function main()
    ...
    opts[:model] = "morphnet4.jl"
    Logging.configure(level=DEBUG)
    Logging.configure(output=open(opts[:logfile], "a"))
    enc_data,c2i,o2i,comp2i = prepare_data(opts)
    ...
    if opts[:mode] == 2
        ...
        besties = zeros(2)
        data  = minibatch(enc_data,opts,o2i,comp2i;st=false)
        words1 = map(x->map(y->y[1],x),enc_data[3])
        words  = map(x->map(y->join(map(z->i2c[z],y),""),x),words1)
        generate(weights,rsettings,data[3],words,i2o,i2oarr,i2c,comp2i,opts)
        return 1
    elseif opts[:mode] == 3
        # transfer learning
        data = minibatch(enc_data,opts,o2i,comp2i)
        rsettings,weights = initweights(opts,c2i,o2i,comp2i,xavier)
        ...
        _,source_c2i,source_o2i,comp2i = prepare_data(source_opts)
    ...
    #=
    dlss,dacc = evaluate(weights,rsettings,data[2],i2o,i2oarr,comp2i,opts)
    info(@sprintf "[dev-0] acc:%.4f lss:%.4f"  (100*dacc[1])/dacc[2] dlss[1]/dlss[2])
    tlss,tacc = evaluate(weights,rsettings,data[3],i2o,i2oarr,comp2i,opts)
...
function evaluate(weights,rsettings,data,i2o,i2oarr,comp2i,opts)
    lssval  = zeros(Float32,2)
    ...
    for i=1:T
        ho[:] = 0.0;co[:] = 0.0
        preds,quantvals = predict(weights,rsettings,data[i],i2o,ho,co,comp2i;lss=lssval)
        unkoutvals += quantvals
    ...
    println("unk token:",unkoutvals[1]," all tokens:",unkoutvals[2]," oran:",100*unkoutvals[1]/unkoutvals[2])
    return lssval,predval
end

function loss(weights,rsettings,data,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=5)
    input,bs,indices,labels,outmasks,prev_tag_ix,decoder_start = data
    total = 0.0; count = 0.0
    ...
    for i=1:wordnumber
        all_ho[i]  = ho[:,:,end]
        all_co[i]  = co[:,:,end]
        #outemb0    = weights[12][:,labels[i,tagrange[i]]]
        outemb0    = weights[12][:,prev_tag_ix[i]]
...
function predict(weights,rsettings,data,rov,ho,co,comp2i;pdrop=0.0,lss=nothing,preds=nothing,prevt=5)
    ...
    for i=1:wordnumber
        ...
        length(o2) == 0 && continue

        outencinp = join(map(x->rov[x],o2),"|")
        current_ix = get(comp2i,outencinp,length(comp2i))
        #println("outencinp:",outencinp)
        if current_ix == length(comp2i)
            unk_out_encoder += 1
        end
        ttl_num += 1

        outemb = weights[12][:,current_ix:current_ix]
        _,ho,co,_ = rnnforw(rsettings[5],weights[9],outemb,ho,co)
    ...
    return preds,[unk_out_encoder,ttl_num]
end

function initweights(o,c2i,o2i,comp2i,init)
    w = Any[]
    rsettings = Any[]
    # embedding
    ...
    # output encoder
    roe,woe = rnninit(o[:es][3],o[:hs][4];rnnType=:lstm,seed=o[:seed]) #9
    push!(rsettings,roe)
    push!(w,wbi)
    # output layer
    ...
    push!(w,init(o[:es][3],length(comp2i))) # 12
...
function minibatch(edf,opts,o2i,comp2i;st=true)
    all_batches = Any[]
    for df in edf
        batches = _minibatch(df,opts,st,o2i,comp2i)
        push!(all_batches,batches)
    end
    return all_batches
end

function _minibatch(df,opts,st,o2i,comp2i)
    batches = Any[]
    ...
    for i=1:length(sorted_df)
        ...
        # bu simdi tagrange degil kendinden onceki tum kelimelerin encodingi gibi olcak
        #tagrange = map(x->(length(x[2])+1):(length(x[2])+1),sorted_df[i])
        prev_tag_ix = map(x->get(comp2i,join(x[3],"|"),length(comp2i)),sorted_df[i])
        ...
        push!(batches,(batch_words,batchsizes,reverse_indices,labels_2d,outmasks_2d,prev_tag_ix,decoder_start))
    end
    return batches
end

function generate(weights,rsettings,data,words,i2o,i2oarr,i2c,comp2i,opts;conlluformat=false)
    lssval = zeros(Float32,2)
    T = length(data)
    if conlluformat
    ...
    println("unk out encoder token:",unkoutvals[1]," total token(repetetive olcak):",unkoutvals[2])
    close(fout)
end
