...
function parseargs()
    ...
    @add_arg_table s begin
        ...
        ("--treebank";arg_type=String; default="trmor2006"; help="trmor2006|trmor2016|trmor2018|saketal")
    end
...
function _minibatch(df,opts,st,o2i)
    ...
    for i=1:length(sorted_df)
        ...
        labels = Any[]
        unknownixs= Int[]
        for k=1:length(sorted_df[i])
            push!(labels,vcat(sorted_df[i][k][2],sorted_df[i][k][3]))
            qq = (opts[:treebank] == "trmor2018") ? "*unknown*" : "*UNKNOWN*"
            if o2i[qq] == sorted_df[i][k][3][1]
                counterdebug += 1
                push!(unknownixs,k)
            end
        end
        ...
        if length(unknownixs) != 0
            outmasks[:,unknownixs] = 0.0
        end
