# sep decs, sep reducers for decs
...
function main()
    ...
    if opts[:mode] == 2
        ...
        info("output(o2i) vocabulary length:",length(o2i))
        info("output(t2i) vocabulary length:",length(t2i))
        besties = zeros(2)
    ...
    elseif isfile(opts[:bestmodel])
        ...
        info("input vocabulary length:",length(c2i))
        info("output vocabulary(o2i) length:",length(o2i))
        info("output(t2i) vocabulary length:",length(t2i))
...
function loss(weights,rsettings,data,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    ...
    lemma_decoder_h1  = relu.((weights[14] * bi_out) .+ weights[15])
    lemma_decoder_h1  = dropout(lemma_decoder_h1,pdrop;training=true)
    lemma_decoder_c1  = KnetArray(zeros(Float32,size(lemma_decoder_h1)))
    tag_decoder_h1  = relu.((weights[16] * bi_out) .+ weights[17])
    tag_decoder_h1  = dropout(tag_decoder_h1,pdrop;training=true)
    tag_decoder_c1  = KnetArray(zeros(Float32,size(tag_decoder_h1)))

    wordnumber = size(lemma_labels,1)
    lemma_outlen = size(weights[10],1)
    tag_outlen   = size(weights[12],1)
    ...
    for i=1:decoder_lemma_ts
        ...
        ypred = weights[10] * hd1_lemma .+ weights[11] #
        # loss calculation
        ynorm = logp(ypred,1)
    ...
    for i=1:decoder_ts
        ...
        rnn2input = dropout(rnn2input,pdrop;training=true)
        yd1,tag_decoder_h1,tag_decoder_c1,_ = rnnforw(rsettings[4],weights[7],
                                                      rnn2input,tag_decoder_h1,tag_decoder_c1)
        ...
        ypred = weights[12] * hd1 .+ weights[13] #
        # loss calculation
        ynorm = logp(ypred,1)
...
function predict(weights,rsettings,data,rov,rtv,ho,co;pdrop=0.0,lss=nothing,preds=nothing,prevt=2)
    ...
    prevPreds = Any[]
    for i=1:wordnumber
        tag_hd1_single = hd1[:,i:i]
        tag_cd1_single = cd1[:,i:i]
        tag_dec_h1_s = tag_decoder_h1[:,i:i]
        tag_dec_c1_s = tag_decoder_c1[:,i:i]
        ...
        for j=1:decoder_tag_ts
            ...
            ypred = weights[12] * y2 .+ weights[13]
            ynorm = logp(ypred,1)
...
function initweights(o,c2i,o2i,t2i,init)
    ...
    rsettings = Any[]
    # embedding
    push!(w,init(o[:es][1],length(c2i))) #1 # word encoder embedding
    push!(w,init(o[:es][2],length(o2i))) #2 # lemma decoder embedding
    push!(w,init(o[:es][3],length(t2i))) #3 # tag deecoder embedding
    # encoder
    ...
    push!(w,wdec2)
    # decoder - tag
    r21,wdec1 = rnninit(o[:es][3],o[:hs][3];rnnType=:lstm,seed=o[:seed]) #7
    push!(rsettings,r21)
    push!(w,wdec1)
    ...
    # output layer - tag
    push!(w,init(length(t2i),o[:hs][3])) #12
    push!(w,zeros(length(t2i),1))        #13
    # lemma bilstm reducer
    push!(w,init(o[:hs][3],2o[:hs][2])) #14
    push!(w,zeros(o[:hs][3],1))         #15
    # tag bilstm reducer
    push!(w,init(o[:hs][3],2o[:hs][2])) #16
    push!(w,zeros(o[:hs][3],1))         #17
    # convert to atype
